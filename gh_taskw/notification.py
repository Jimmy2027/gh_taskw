...
class GhUrl(HttpUrl):
    @field_validator("url")
    @classmethod
    def convert_api_url(cls, v: str) -> str:
        return (
            v.replace("api.", "").replace("/repos/", "/").replace("/pulls/", "/pull/")
        )
...
class Subject(BaseModel):
    ...
    @field_validator("url")
    @classmethod
    def convert_api_url(cls, v: Url | None) -> Url:
        if v is None:
            return v
        return Url(
            v.__str__()
...
class GhNotification(BaseModel):
    ...
    @field_validator("url")
    @classmethod
    def convert_api_url(cls, v: Url) -> Url:
        return Url(
            v.__str__()
            .replace("api.", "")
            .replace("/repos/", "/")
