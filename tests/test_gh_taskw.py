...
def get_log():
    logfn = Path("~/.local/logs/gh_notifications.json").expanduser()
    ...
    resons = {( log.reason.value ,log.subject.type.value ) for log in logs_list}
    # sort resons by log.reason.value
    resons = sorted(resons, key=lambda x: x[0])
